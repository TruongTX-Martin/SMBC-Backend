...
class Enum(AbstractRule):
    ...
    def validate(self, value):
        ...
        if value not in self.allowed_values:
            errors.append('Incorrect value "{}". Allowed values: {}'.format(
...
class MaxLength(AbstractRule):
    ...
    def validate(self, value):
        ...
        if len(value) > self.length:
            errors.append(
                'Invalid length for value "{}". Max length = {}'.format(
                    value, self.length))
        return errors
...
class MinLength(AbstractRule):
    ...
    def validate(self, value):
        ...
        if len(value) < self.length:
            errors.append(
                'Invalid length for value "{}". Min length = {}'.format(
                    value, self.length))
        return errors
...
class Email(AbstractRule):
    def __init__(self):
        ...
        self.pattern = re.compile(
            r'^[a-z][a-z0-9_\.]{0,32}@[a-z0-9]{2,}(\.[a-z0-9]{2,4}){1,2}$')
